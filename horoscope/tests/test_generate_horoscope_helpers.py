...
class TestSendDailyHoroscope:
    ...
    def test_subscriber_receives_full_text(self):
        ...
        mock_user_profile_repo.get_by_telegram_uid.return_value = _make_profile()
    ...
    @pytest.mark.django_db
    def test_non_subscriber_receives_teaser_with_subscribe_link(self):
        from horoscope.tasks.generate_horoscope import _send_daily_horoscope
    ...
    @pytest.mark.django_db
    def test_failed_send_marks_horoscope_as_failed(self):
        from horoscope.tasks.generate_horoscope import _send_daily_horoscope
        ...
        with patch('core.containers.container') as mock_container, \
             patch('horoscope.tasks.messaging.send_message', return_value=False):
            mock_container.horoscope.horoscope_repository.return_value = mock_horoscope_repo
            mock_container.horoscope.subscription_repository.return_value = mock_subscription_repo
            mock_container.horoscope.user_profile_repository.return_value = mock_user_profile_repo
        ...
        mock_horoscope_repo.mark_sent.assert_not_called()
        mock_horoscope_repo.mark_failed_to_send.assert_called_once_with(horoscope_id=42)

    @pytest.mark.django_db
    def test_uses_profile_language_for_translation(self):
        from horoscope.tasks.generate_horoscope import _send_daily_horoscope
...
class TestSendFirstHoroscope:
    @pytest.mark.django_db
    def test_sends_first_horoscope_success(self):
    ...
    @pytest.mark.django_db
    def test_sends_first_horoscope_failure(self):
        from horoscope.tasks.generate_horoscope import _send_first_horoscope
    ...
    @pytest.mark.django_db
    def test_defaults_to_en_when_no_profile(self):
        from horoscope.tasks.generate_horoscope import _send_first_horoscope

        mock_horoscope_repo = MagicMock()

        mock_user_profile_repo = MagicMock()
        mock_user_profile_repo.get_by_telegram_uid.return_value = None
